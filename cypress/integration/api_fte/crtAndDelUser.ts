...
describe(['my-feature1'],'Create and delete user', () => {
    it.only(['E2E'],'Call crt-user then dlt-User with verifications', () => {
        cy.crtUser(fixtureBody).then((response:any) => {
            const rBody = Cypress._.map(response.body)
            const uId: any = rBody[4]
            const name = rBody[0]
            ...
            cy.dltUser(uId).then((response:any) => {
